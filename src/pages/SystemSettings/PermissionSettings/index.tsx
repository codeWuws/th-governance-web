...
import { Card, Tree, Select, Space, Button, message, Spin, Input, Tag, Badge, Typography, Progress } from 'antd'
...
const PermissionSettings: React.FC = () => {
  ...
  const groupDefs = useMemo(() => (
    [
      { key: '数据治理', prefixes: ['/dashboard', '/database-connection', '/data-governance'] },
  ...
  const groupedAll = useMemo(() => {
    ...
    groupDefs.forEach(def => { g[def.key] = [] })
      ...
      <Card className={styles.headerCard} bordered={false} bodyStyle={{ padding: 0 }}>
        <div className={styles.header}>
          <Space direction="vertical" size={4}>
            <div className={styles.title}><SafetyOutlined /> 权限管理</div>
            <div className={styles.subtitle}>按左侧菜单进行授权，精确控制角色可访问的模块与页面</div>
          </Space>
              ...
              {roles.length ? (
                roles.map(r => (
                  <Tag
                    ...
                    style={{ cursor: 'pointer', fontWeight: selectedRoleId === r.id ? 600 : 400 }}
                  >
                    {r.name}（{r.code}）
                  </Tag>
                ))
            ...
            {metrics.map(m => (
              <div key={m.name} className={styles.metricCard}>
                ...
                <div>
                  ...
                  <div className={styles.metricValue}>授权 {m.used} / {m.total}</div>
                </div>
              </div>
            ))}
          </div>

          <Card className={styles.treeCard} bodyStyle={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
                ...
                <Tree
                  ...
                  titleRender={(node) => (
                    <Space size={6}>
                      ...
                      {typeof node.key === 'string' && String(node.key).startsWith('/') ? (
                  ...
                  filterTreeNode={(node) => {
                    ...
                    return title.toLowerCase().includes(search.toLowerCase()) || key.toLowerCase().includes(search.toLowerCase())
                  }}
                />
              </div>
            </Spin>
